...
Retail (3.7.0) adds `Healium.HostileProfiles[specIndex]`, the same shape, feeding frames flagged `isHostile` (the Arena frame). Always resolve a button's profile with `Healium_GetProfileForFrame(frame)` / `Healium_GetProfileForButton(button)`, never `Healium_GetProfile()` directly; the config panel edits whichever set its "Button Set" dropdown selects.

The offered spells are two separate lists, not one: `Healium_Spell` (cast on a friend) and `Healium_HostileSpell` (cast on an enemy), selected with `Healium_GetSpellList(hostile)` and both filled by `Healium_InitSpells`. Keeping them apart is what stops the party dropdowns offering Polymorph. The hostile list covers all thirteen classes and is deliberately narrow: offensive dispel, interrupt, targeted crowd control, and debuffs applied to a second enemy. `HealiumSpells.lua` also keeps an `OffensiveCures` table, read through `Healium_GetOffensiveDispelTypes`, holding the five Magic dispels that exist on enemies (Priest, Shaman, Mage, Demon Hunter, Hunter). Enrage-only removals such as Soothe and Shiv are absent on purpose, since no aura filter can match them.
